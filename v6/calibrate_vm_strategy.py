...
def load_calibration_data(rows_to_load: int, end_at_row: int) -> pd.DataFrame:
    ...
    """
    # skip is the number of data rows to skip from the start (row 1 onwards)
    ...
    if skip > 0:
        # skiprows=[1, 2, ..., skip] skips the oldest 'skip' data rows, but keeps header (row 0)
        read_kwargs["skiprows"] = list(range(1, skip + 1))
        
    logger.info(f"Reading {rows_to_load:,} rows starting from row {skip + 1:,} (most recent training window)")
    data = pd.read_csv(DATA_CSV, **read_kwargs)
    
    # Fix potential header issues after skiprows
...
def generate_vm_signals(data: pd.DataFrame, p: dict, trial_id: str = "trial") -> pd.DataFrame:
    # Use config defaults for anything not in 'p'
    full_p = {**VM_DEFAULTS, **p}

    multi_rsi = calculate_multi_rsi_plus(
        data, 
        p1=full_p["multi_rsi_p1"], 
        p2=full_p["multi_rsi_p2"], 
        p3=full_p["multi_rsi_p3"], 
        smal=full_p["multi_rsi_smal"]
    )

    cyclic_rsi = calculate_cyclic_rsi(
        data, 
        dom_cycle=full_p["crsi_dom_cycle"], 
        vibration=full_p["crsi_vibration"], 
        leveling=full_p["crsi_leveling"]
    )

    stoch_macd = calculate_stochastic_macd(
        data, 
        fast_len=full_p["stoch_macd_fast_len"], 
        slow_len=full_p["stoch_macd_slow_len"], 
        signal_len=full_p["stoch_macd_signal_len"], 
        lookback=full_p["stoch_macd_lookback"],
        fast_len_xd=full_p["stoch_macd_fast_len_xd"],
        slow_len_xd=full_p["stoch_macd_slow_len_xd"],
        signal_len_xd=full_p["stoch_macd_signal_len_xd"],
        lookback_xd=full_p["stoch_macd_lookback_xd"]
    )
    ...
    stoch_bull = recent(stoch_bull_raw, full_p["confirmation_lookback_bars"])
    ...
    # Exit condition: Stochastic MACD crossunder
    stoch_exit = crossed_below(stoch_macd["Slow_MACD"], stoch_macd["Slow_Signal"])

    timestamps = data["timestamp"].to_numpy()
    buy_arr = clean_bool(buy_setup).to_numpy()
    exit_arr = clean_bool(stoch_exit).to_numpy()
...
def run_calibration(smoke: bool = False):
    ...
    for t in tqdm(range(p1_trials), desc="Phase 1"):
        ...
        signals = generate_vm_signals(data_small, trial_params, trial_id=f"P1-{t+1}")
    ...
    for c_idx, cand in enumerate(tqdm(top_candidates, desc="Phase 2 Candidates")):
        for t in tqdm(range(p2_trials_per), desc=f"Refining C{c_idx+1}", leave=False):
            ...
            signals = generate_vm_signals(data_small, trial_params, trial_id=f"P2-C{c_idx+1}-T{t+1}")
            score = get_score(data_small, signals) if not signals.empty else -np.inf
    ...
    for t in tqdm(range(p3_trials), desc="Phase 3"):
        ...
        signals = generate_vm_signals(data_full, trial_params, trial_id=f"P3-{t+1}")
    ...
    final_params = round_final_params({**VM_DEFAULTS, **best_final["params"]}, ranges)
    results = {
        "best_parameters": final_params,
        "best_score": best_final["score"],
        "calibration_date": datetime.now().isoformat(),
        "dataset_info": {"p1_p2_rows": trial_rows, "p3_rows": total_cal_rows, "strategy": "vm"}
    }
